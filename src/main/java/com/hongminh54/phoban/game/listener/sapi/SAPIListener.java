package com.hongminh54.phoban.game.listener.sapi;
...
public class SAPIListener implements Listener {
    ...
    private boolean predictDeath(SkillDamageEvent e) {
    ...
    private boolean predictDeath(TrueDamageEvent e) {
        Entity entity = e.getTarget();
        if(!(entity instanceof Player)) return false;
        Player p = (Player) entity;
        boolean predict = p.getHealth() <= e.getDamage();
    ...
    public void damageCountSkill(SkillDamageEvent e) {
        if(e.getTarget() instanceof LivingEntity && e.getDamager() instanceof Player) {
            BukkitAPIHelper mm = AEPhoBan.inst().getBukkitAPIHelper();
            if(!mm.isMythicMob(e.getTarget())) return;
